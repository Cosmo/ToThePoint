...
(function ($) {
  $.fn.tldw = function(options){
    ...
    options.selectedObject.addClass("video");
...
chrome.extension.sendMessage({}, function(response) {
    ...
    $(function(){
      ...
      if($("#spVideoElements").length > 0) {
        $("#spVideoElements").find("object").remove();
        $("<video src='http://video2.spiegel.de/flash/45/08/1528054_iphone.mp4'></video>").appendTo($("#spVideoElements"));
      }
      
      if($(".html5-video-container").length > 0) {
        $(".html5-video-container").tldw({
          signals: [
            60, 75, 81, 86,
            240, 256, 270,
            360, 361, 377, 390, 398, 411,
            480, 509
          ]
        });
      }
